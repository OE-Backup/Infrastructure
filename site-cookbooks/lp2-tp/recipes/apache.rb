...
#
# TP webapp uses apache2 default feature set plus some modules
...
tp_ssl_info = Chef::DataBagItem.load("lp2", "ssl")
tp_secrets = Chef::EncryptedDataBagItem.load("lp2", "secrets")
...
%w(ca crt key).each do |pem|
  file "#{node['apache']['dir']}/sites-conf/default/ssl/wildcard.#{node['lp2']['domain']}.#{pem}" do
    ...
    case pem
    when "ca"
      content tp_ssl_info[node.chef_environment]['chainfile']
      mode "0644"
    when "crt"
      content tp_ssl_info[node.chef_environment]['wildcard_cert']
      mode "0644"
    when "key"
      content tp_secrets[node.chef_environment]['wildcard_key']
      mode "0400"
    else
    end
  end
...
if node['lp2']['maintenance_mode']
  include_recipe "lp2-tp::maintenance"
else
