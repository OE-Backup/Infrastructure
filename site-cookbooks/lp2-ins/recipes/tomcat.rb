...
[ "postgresql-9.2-1002-jdbc4.jar", \
  ...
  remote_file "#{node["tomcat"]["lib_dir"]}/#{jar}" do
    ...
    mode 0644
    source "#{node["lp2"]["ins"]["jars_url"]}/#{jar}"
  end
end
...
template "#{node["tomcat"]["config_dir"]}/context.xml" do
  ...
  variables({
    :jdbc_user       => lp2_settings[node.chef_environment.to_s]["ins"]["jdbc_user"],
    :jdbc_pass       =>  lp2_secrets[node.chef_environment.to_s]["ins"]["jdbc_pass"],
    :jdbc_url        => lp2_settings[node.chef_environment.to_s]["ins"]["jdbc_url"],
  })
end
...
#if node["lp2"]["ins"][node.chef_environment.to_s]["war_url"]
#  remote_file "#{node["tomcat"]["webapp_dir"]}/LP2TOSalesForceIntegration.war" do
#    owner node["tomcat"]["user"]
#    group node["tomcat"]["group"]
#    mode 0644
#    source node["lp2"]["ins"][node.chef_environment.to_s]["war_url"]
#    checksum node["lp2"]["ins"][node.chef_environment.to_s]["war_checksum"]
#  end
#end
